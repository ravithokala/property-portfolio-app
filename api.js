  ...
  function create(config) {
    ...
    return {
      ...
      async write(action,payload) {
        if(!['company_compliance.create','company_compliance.update','maintenance.create','maintenance.update','property.update','mortgage.update','tenancy.update','compliance.update'].includes(action))return failure('BAD_REQUEST');
        const session=read();
        if(!session)return failure('UNAUTHENTICATED');
        let result;
